...
> = async (req, res, next) => {
  ...
  try {
    ...
    const confirmLink = `${env.WEBSITE_URL}/new-verification?token=${token}`;
    const resend = new Resend(env.RESEND_API_KEY);
    const { error } = await resend.emails.send({
      from: "Adakan Library Core <adakansoftware@gmail.com>",
      to: email,
      subject: "Adakan Library Core hesabini onayla",
...
export const resetPassword: RequestHandler = async (req, res, next) => {
  const userInputValue =
    req.body.userInputValue || req.body.email || req.body.user_name;
  ...
  try {
    ...
    const { error } = await resend.emails.send({
      from: "Adakan Library Core <adakansoftware@gmail.com>",
      to: user.user_email,
      subject: "Adakan Library Core sifreni sifirla",
